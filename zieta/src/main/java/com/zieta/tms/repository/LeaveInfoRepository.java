...
public interface LeaveInfoRepository extends JpaRepository<LeaveInfo, Long> {
	...
	@Query(value ="select * from leave_info ln where ln.client_id=?1 and ln.user_id in ?2 and ln.is_delete= ?3 and ln.leave_start_date >= ?4 and ln.leave_start_date <= ?5", nativeQuery=true)
